...
function AdminDashboard() {
  const { usuarios, citas, preCitas } = useAppContext();
  ...
  const getRecentActivity = () => {
    ...
    recentClients.slice(0, 2).forEach((client) => {
      const hoursAgo = Math.max(
        1,
        Math.floor(
          (now.getTime() - new Date(client.fechaRegistro).getTime()) /
            (1000 * 60 * 60),
        ),
      );
      activities.push({
    ...
    recentAppointments.slice(0, 2).forEach((appointment) => {
      const hoursAgo = Math.max(
        1,
        Math.floor(
          (now.getTime() - new Date(appointment.fecha).getTime()) /
            (1000 * 60 * 60),
        ),
      );
      activities.push({
    ...
    recentPreCitas.slice(0, 1).forEach((preCita) => {
      const hoursAgo = Math.max(
        1,
        Math.floor(
          (now.getTime() - new Date(preCita.fechaCreacion || now).getTime()) /
            (1000 * 60 * 60),
        ),
      );
      activities.push({
