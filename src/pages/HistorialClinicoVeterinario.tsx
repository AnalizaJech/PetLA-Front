                    ...
                    return (
                      ...
                      >
                        <CardContent className="p-6">
                          ...
                          {/* Información completa de la mascota, propietario y cita */}
                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6 p-4 bg-vet-gray-50 rounded-lg">
                            {/* Datos de la mascota */}
                            <div>
                              <h5 className="font-medium text-vet-gray-900 mb-2 flex items-center">
                                <PawPrint className="w-4 h-4 mr-2 text-vet-primary" />
                                Datos de la Mascota
                              </h5>
                              <div className="space-y-1 text-sm text-vet-gray-700">
                                <div><strong>Nombre:</strong> {selectedPet.nombre}</div>
                                <div><strong>Especie:</strong> {selectedPet.especie || "No especificada"}</div>
                                <div><strong>Raza:</strong> {selectedPet.raza || "No especificada"}</div>
                                <div><strong>Sexo:</strong> {selectedPet.sexo || "No especificado"}</div>
                                {selectedPet.fechaNacimiento && (
                                  <div><strong>Fecha Nac.:</strong> {new Date(selectedPet.fechaNacimiento).toLocaleDateString("es-ES")}</div>
                                )}
                                {selectedPet.microchip && (
                                  <div><strong>Microchip:</strong> {selectedPet.microchip}</div>
                                )}
                                {record.peso && (
                                  <div><strong>Peso actual:</strong> {record.peso} kg</div>
                                )}
                              </div>
                            </div>
                            ...
                            <div>
                              <h5 className="font-medium text-vet-gray-900 mb-2 flex items-center">
                                <UserCheck className="w-4 h-4 mr-2 text-vet-primary" />
                                Datos del Propietario
                              </h5>
                              <div className="space-y-1 text-sm text-vet-gray-700">
                                {selectedOwner ? (
                                  <>
                                    <div><strong>Nombre:</strong> {selectedOwner.nombre} {selectedOwner.apellidos || ''}</div>
                                    {selectedOwner.telefono && (
                                      <div className="flex items-center space-x-1">
                                        <Phone className="w-3 h-3" />
                                        <span><strong>Teléfono:</strong> {selectedOwner.telefono}</span>
                                      </div>
                                    )}
                                    {selectedOwner.email && (
                                      <div className="flex items-center space-x-1">
                                        <Mail className="w-3 h-3" />
                                        <span><strong>Email:</strong> {selectedOwner.email}</span>
                                      </div>
                                    )}
                                    {selectedOwner.direccion && (
                                      <div className="flex items-center space-x-1">
                                        <MapPin className="w-3 h-3" />
                                        <span><strong>Dirección:</strong> {selectedOwner.direccion}</span>
                                      </div>
                                    )}
                                  </>
                                ) : (
                                  <div>Propietario no registrado</div>
                                )}
                              </div>
                            </div>
                            ...
                            <div>
                              <h5 className="font-medium text-vet-gray-900 mb-2 flex items-center">
                                <Info className="w-4 h-4 mr-2 text-vet-primary" />
                                Detalles de la Cita
                              </h5>
                              <div className="space-y-1 text-sm text-vet-gray-700">
                                <div><strong>Tipo de Servicio:</strong> {record.tipo || record.tipoConsulta}</div>
                                <div><strong>Fecha/Hora:</strong> {new Date(record.fecha).toLocaleDateString("es-ES")} a las {new Date(record.fecha).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })}</div>
                                <div><strong>Estado:</strong>
                                  <span className="ml-1 px-2 py-1 rounded-full text-xs bg-green-100 text-green-800">
                                    {record.estado === "completada" ? "Completada" : record.estado}
                                  </span>
                                </div>
                                <div><strong>Veterinario:</strong> Dr. {record.veterinario}</div>
                                {record.citaId && (
                                  <div><strong>ID Cita:</strong> {record.citaId}</div>
                                )}
                              </div>
                            </div>
                          </div>
